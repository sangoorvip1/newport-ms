...
```bash
npm run test:all          # domain 82 · api 56 (منها 4 على قاعدة حيّة) · mobile 8  = 146 فحصًا
...
- `GET /` على منفذ الـAPI يعيد بطاقة تعريف الخدمة (صفحة عربية في المتصفح، JSON للعملاء الآليين) بدل
  `Cannot GET /` — فلا يخلط المشغّل بين «خدمة ميتة» و«مسار غير معرَّف». الواجهة البشرية ليست هنا.
- طابور رفع الوثائق في العميلين (SQLite/Dexie): فحص محلي للنوع والحجم قبل الشبكة، نفس معرّف العميل في كل
  إعادة محاولة، ورفض `push: document` في المزامنة (restOnly ⇒ `REJECTED` في الفحص الحيّ).
