...
> الأرقام مقاسة على HEAD بتاريخ 2026-09-08: **165** فحص وحدة · **83/83** فحص دخان حيّ · typecheck نظيف.
...
بعد كل نشر: `npm run e2e -w @newport/api` مقابل الخادم الحيّ → **83/83** هو خط القبول.
...
كلمة المرور الموحّدة: `Newport#2026` (تُستبدل بـ`SEED_DEFAULT_PASSWORD=...` قبل البذر). كل حساب يُبذَر
جديدًا بـ`mustChangePwd=true` فيُجبَر على تغييرها أول دخول، وينطفئ العلم لذلك الحساب وحده بعد التغيير.

> قِيس 2026-09-08: إعادة البذر فوق قاعدة **موجودة** تُعيد كلمات المرور إلى الافتراضية (201 لكل الحسابات)
> لكنها **لا تُعيد رفع** `mustChangePwd` — العلم يُضبط عند الإنشاء فقط. لمحاكاة «أول دخول» كاملة: أوقف
> نصّ القاعدة بـ`Ctrl-C`، ثم احذف `apps/api/.pgdata`، ثم شغّل `scripts/dev.ps1 -LocalDb` (أو `dev-db.mjs`).
> لهذا السبب لا تُشغّل `npm run e2e -w @newport/api` قبل عرض كلمات المرور على عميل: الدخان يُغيّرها بحساباته الأربعة.
...
```bash
npm run test:all        # 165 فحصًا (domain 94 · api 63 · mobile 8)
