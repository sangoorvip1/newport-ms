...
- **طابور الرفع في العملاء (`DocumentUploadQueue` في `@newport/domain`)**: `SYNC_META.document.restOnly = true`
  ⇒ `SyncClient.queue()` يرفض كتابته والـ API يعيد `REJECTED` لأي عملية دفع على `document` (فحص حيّ في
  `e2e-smoke`). البايتات تبقى في مخزن العميل (SQLite/Dexie) حتى يقبل الخادم السطر، ومعرّف العميل يُعاد في كل
  محاولة فلا يتكرر السطر عند انقطاع منتصف الرفع. الهاتف يستعمل `POST upload` (JSON) لأن RN لا يرسل ثنائيًا
  بلا Blob/ملف، والمكتب يستعمل `presign`+`PUT` بجسم `Uint8Array` (بلا تضخيم 33%).
- **الالتقاط بلا بايتات لا يُخزَّن**: مصغّرة وحدها لا تُنشئ سطر `documents` (كان الطابور القديم يرسل
  `objectKey: offline/...` فيبدو المرفق موجودًا وتحميله 404 للأبد) — العميل يعيد رسالة عربية للمستخدم.
